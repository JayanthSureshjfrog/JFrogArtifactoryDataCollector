...
###################################
Check_Before_Run
printf " ###########################\e[1;31mThis script will not consider any default values, so please pass the values explicitly \e[0m########################### \n"
echo -en "\e[1;33mPlease enter the Artifactory Home path (The path till var and app folder, Ex: /opt/jfrog/artifactory):\e[0m"
read ARTIFACTORY_HOME_PATH
if [ -z $ARTIFACTORY_HOME_PATH ]; then 
export ARTIFACTORY_HOME_PATH="/opt/jfrog/artifactory" ;
echo -e "\e[1;32mDefault valus is $ARTIFACTORY_HOME_PATH \e[0m";
fi
if [ ! -d $ARTIFACTORY_HOME_PATH ]; then 
echo -en "\e[1;31m $ARTIFACTORY_HOME_PATH path does not exist\e[0m"
exit 1 
fi
echo -en "\e[1;33mPlease enter the target path to save the files (Ex: /tmp):\e[0m"
read FILE_PATH
if [ -z $FILE_PATH ]; then 
export FILE_PATH="/tmp";
echo -e "\e[1;32mDefault valus is $FILE_PATH \e[0m";
fi
if [ ! -d $FILE_PATH ]; then 
echo -en "\e[1;31m $FILE_PATH path does not exist\e[0m"
exit 1 
fi
echo -en "\e[1;33mPlease enter the DB port (\e[1;33mEx: 5432 for postgres or leave blank for derby DB):\e[0m"
read DB_PORT
echo -e "\e[1;32mDefault valus is NULL \e[0m";
echo -en "\e[1;33mPlease enter the ticket number to upload the collected data to supportlogs.jfrog.com (leave blank to ignore the upload):\e[0m"
read TICKET_NUMBER
echo -e "\e[1;32mDefault valus is NULL \e[0m";
echo -en "\e[1;33mWould you like to capture the Heap Dump? It would consume extra resource and space (size of the max. heap size) - y/n : \e[0m"
read HEAP_DUMP_FLAG
echo -e "\e[1;32mDefault valus is n \e[0m";
echo -en "\e[1;33mWould you like to capture the Disk throughput and latency? It would consume extra Server Memory - y/n : \e[0m"
read DISK_DETAILS_FLAG
echo -e "\e[1;32mDefault valus is n \e[0m";
echo -en "\e[1;33mHow many thread dumps would you like to take - (5 or 10): \e[0m"
read TD_COUNT
if [ -z $TD_COUNT ] || [ $TD_COUNT -lt 0 ];then
echo -en "\e[1;31m Invalid or NULL value, please pass the value greater then zero \e[0m" 
exit 1
fi
echo -en "\e[1;33mPlease specify the time interval between each thread dump - (5 or 10) in seconds: \e[0m"
read TD_TIME_INTERVAL
